...
void DrawGl::drawWinners() {
    ...
    begin2d();
    glBegin(GL_QUADS);
        glVertex2d(this->width() / 2 - 150, this->height() - 20 * a->otherHeroes);
        glTexCoord2d(0, 1);
        glVertex2d(this->width() / 2 + 150, this->height() - 20 * a->otherHeroes);
        glTexCoord2d(1, 1);
        glVertex2d(this->width() / 2 + 150, this->height() + 20 * a->otherHeroes);
    ...
    for (int i = 0; i < a->winners.size(); i++)
        renderText(this->width() / 2 - 120, 15 + 20 * i, QString::number(i + 1) + ": " + a->winners[i] + "(finished)", hudFont);

    int current = a->winners.size();
    for (int i = a->otherHeroes - 1; i >= 0; i--)
        if (!a->winners.contains(a->players[i].second)) {
            renderText(this->width() / 2 - 120, 15 + 20 * current, QString::number(current + 1) + ": " + a->players[i].second + "(" +
